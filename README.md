...
- #### Назови все основные компоненты Android приложения? (4)
  ...
  3. **Broadcast Receiver** - принимает и обрабатывает броадкаст ивенты, отправленные внутри приложения или из других приложений.
  4. **Content Provider** - используется для обмена данными с другими приложениями.
...
- #### Описание MVP
  - **Model** представляет бизнес-логику и данные. Отвечает за доступ к данным, их обработку и отправку.
  - **View** отвечает за отображение данных пользователю и перенаправление пользовательских действий в Presenter.
  - **Presenter** слой посредник, который обрабатывает всю логику взаимодействия между View и Model. Получает действия от View, работает с Model и обновляет View.
![MVVM](/img/mvp.webp)
...
- #### Описание MVI
  ...
  - **View** отображает состояние модели пользователю и генерирует `Intent`ы на основе взаимодействий пользователя.
...
- #### L — Принцип подстановки Барбары Лисков (Liskov Substitution Principle)
  Подклассы должны быть заменяемы на свои базовые классы без изменения корректности программы.
  ```kotlin
    // Нарушение принципа
    open class Bird {
        open fun fly() {}
    ...
    class Duck : Bird() {
        override fun fly() { /* Утка летает */ }
    }
  
    class Ostrich : Bird() {
        override fun fly() { /* Страус не летает, нарушение принципа */ }
    }
    ...
    class Ostrich : Bird() { /* Страус не летает и не наследуется от FlyingBird */ }

  ```
- #### I — Принцип разделения интерфейса (Interface Segregation Principle)
  Клиенты не должны быть вынуждены имплементировать интерфейсы, которые они не используют.
  ```kotlin
  // Нарушение принципа
  interface Worker {
    fun work()
